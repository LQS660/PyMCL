...
class AccountPage(QWidget):
    def __init__(self, backend, parent=None):
        ...
        root.addWidget(CaptionLabel("微软正版、离线、Little Skin / 自建 Yggdrasil 皮肤站"))
        ...
        btn = PrimaryPushButton(FIF.PEOPLE, "设备码登录")
    ...
    def reload(self):
        ...
        for row in rows:
            ...
            kind = "微软" if row["type"] == "microsoft" else ("皮肤站" if row["type"] == "authlib" else "离线")
            bar.addWidget(Pill(kind, "#2E9B6B" if row["type"] == "microsoft" else "#7C5CD6"))
            if row.get("active"):
    ...
    def _offline(self):
        ...
        if not name:
            ...
            return
        self.backend.add_offline_account(name)
        self.reload()
