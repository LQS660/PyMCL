...
class BackendAPI(QObject):
    ...
    def set_account_skin(self, name: str, path: str = "", model: str = "classic",
        ...
        1.19.3 起内置默认皮肤有九种，靠挑 UUID 凑 Steve/Alex 的老办法已经不成立；
        真皮肤由启动时拉起的本地 Yggdrasil 服务发出去，见 mclauncher/skinserver.py。
        """
        import base64
        import binascii
        from mclauncher import skin as skin_mod
    ...
    # ==================================================================
    # 新增 API：启动页自定义布局
    # 与 bridge/api.py 同名同义，保持两套后端方法面一致。Qt 界面自己直接
    # 用 layout_model（= mclauncher.ui_layout），这组方法主要给 AI 工具层 /
    # 外部调用者用；数据落在同一组 config.json 键上。
    # ==================================================================
    ...
    def import_layout(self, doc: dict) -> dict:
        """导入一份布局文档（已解析的 JSON）。未知卡片类型丢弃，结构不对抛错。"""
        from mclauncher import ui_layout as lm
        parsed = lm.parse_doc(doc)
        if parsed is None:
