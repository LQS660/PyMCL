...
def read_game_servers(path) -> list:
    """读原版 servers.dat，给启动器「服务器」页用。"""
    return _read_servers(Path(path))


def write_game_servers(path, servers: list) -> Path:
    dest = Path(path)
    utils.ensure_dir(dest.parent)
    _write_servers(dest, servers)
    return dest
