...
public sealed class ServersPage : PageBase
{
    ...
    public ServersPage()
    {
        var add = Ui.Btn("添加服务器", BtnKind.Primary, (_, _) => Run(() => EditAsync(null, -1)), Ico.Add);
        var imp = Ui.Btn("导入", BtnKind.Chip, (_, _) => Run(ImportAsync), Ico.Import);
        var exp = Ui.Btn("导出", BtnKind.Chip, (_, _) => Run(ExportAsync), Ico.Export);
        var head = Ui.Section("服务器", "直接写进实例的 servers.dat，进游戏就能看到",
            Ui.H(8, _count.VCenter(), imp, exp, add));
        _inst.SelectionChanged += (_, _) => Run(ReloadAsync);
        var bar = Ui.Card(Ui.H(10,
            Ui.Txt("实例", 12, fg: "B.InkMuted").VCenter(), _inst.W(200), _lan.VCenter()), 12);
    ...
    private async Task ReloadAsync()
    {
        ...
        _count.Text = $"{rows.Count} 个";
        if (rows.Count == 0)
        {
            _list.Children.Add(Ui.Empty(Ico.Server, "还没有服务器", "点右上角添加，或从 servers.txt 导入"));
            return;
        }
        for (var i = 0; i < rows.Count; i++)
        {
            ...
            g.Add(Ui.H(6,
                Ui.Btn("进入", BtnKind.Soft, (_, _) => Run(() => JoinAsync(r)), Ico.Play),
                Ui.Btn("编辑", BtnKind.Chip, (_, _) => Run(() => EditAsync(r, idx))),
                Ui.Btn("删除", BtnKind.Ghost, (_, _) => Run(async () =>
                {
                    if (!await Dlg.Confirm("删除服务器", r.Name, "删除", "取消", true)) return;
    ...
    private async Task JoinAsync(ServerRow row)
    {
        ...
        if (versions.Count == 0)
        {
            Toast("这个实例没有版本", "先去下载页装一个", ToastKind.Warning);
            return;
        }
        var pick = Ui.Combo(versions);
        if (!await Dlg.Ask($"进入 {row.Name}", Ui.V(8, Ui.Muted($"{row.Ip}:{row.Port}"), pick), "启动并直连")) return;
        var accounts = await Api.TryCallAsync<List<string>>("get_accounts", null, new()) ?? new();
        await Api.StartTaskAsync("launch_game", new
        {
            instance = inst,
            version = pick.Str(),
            account = accounts.FirstOrDefault() ?? "离线模式",
    ...
    {
        var inst = _inst.Str();
        var name = Ui.Input("显示名", row?.Name ?? "");
        var ip = Ui.Input("mc.example.com", row?.Ip ?? "");
        var port = Ui.Input("25565", (row?.Port ?? 25565).ToString());
        var desc = Ui.Input("备注（可空）", row?.Description ?? "");
        var body = Ui.V(8, name, ip, port, desc);
        if (!await Dlg.Ask(row is null ? "添加服务器" : "编辑服务器", body, "保存")) return;
    ...
    {
        var file = Dlg.PickFile("服务器列表 (*.txt)|*.txt|全部文件|*.*", "导入服务器");
        ...
        Toast("已导入", $"{n} 个服务器", ToastKind.Success);
        await ReloadAsync();
    }
    ...
    private async Task ExportAsync()
    {
        ...
        var dest = Dlg.SaveFile("服务器列表 (*.txt)|*.txt", "servers.txt", "导出服务器");
        if (dest is null) return;
        await System.IO.File.WriteAllTextAsync(dest, text);
        Toast("已导出", dest, ToastKind.Success);
    }
