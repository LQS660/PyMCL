...
function render(container: HTMLElement) {
  ...
  container.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:16px">
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        ...
        <select class="select" id="java-vendor"><option value="adoptium">Adoptium</option><option value="zulu">Zulu</option><option value="graal">GraalVM</option><option value="oracle">Oracle</option></select>
  ...
  ['8', '11', '17', '21'].forEach(major => {
    document.getElementById(`btn-download-java-${major}`)?.addEventListener('click', async () => {
      ...
      try {
        const vendor = (document.getElementById('java-vendor') as HTMLSelectElement | null)?.value || 'adoptium';
