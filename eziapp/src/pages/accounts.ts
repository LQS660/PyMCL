...
function render(container: HTMLElement, reload: () => Promise<void>) {
        ...
        <div style="border-top:1px solid var(--border-light);padding-top:16px">
          ...
          <div class="form-row" style="align-items:flex-end">
            <div class="form-group" style="min-width:180px">
              <label class="form-label">常用皮肤站</label>
              <select class="select" id="authlib-preset" style="width:100%"><option value="">手动填写</option></select>
            </div>
          ...
          <div style="font-size:13px;font-weight:600;margin:16px 0 10px">统一通行证（Nide8）</div>
          <div class="form-row" style="align-items:flex-end">
            <div class="form-group" style="flex:1"><label class="form-label">服务器 ID / 链接</label><input class="input" id="nide8-id" style="width:100%"></div>
            <div class="form-group" style="flex:1"><label class="form-label">用户名</label><input class="input" id="nide8-user" style="width:100%"></div>
            <div class="form-group" style="flex:1"><label class="form-label">密码</label><input class="input" id="nide8-pass" type="password" style="width:100%"></div>
            <button class="btn" id="account-add-nide8">登录</button>
          </div>
        </div>
      </div>
  ...
  void bridge.call<Array<{ name?: string; api?: string }>>('authlib_presets').then((rows) => {
    const sel = container.querySelector<HTMLSelectElement>('#authlib-preset');
    if (!sel) return;
    for (const row of rows || []) {
      const opt = document.createElement('option');
      opt.value = row.api || '';
      opt.textContent = row.name || row.api || '';
      sel.appendChild(opt);
    }
    sel.addEventListener('change', () => {
      const api = container.querySelector<HTMLInputElement>('#authlib-api');
      if (api && sel.value) api.value = sel.value;
    });
  }).catch(() => undefined);

  container.querySelector<HTMLButtonElement>('#account-add-nide8')?.addEventListener('click', async () => {
    const server_id = container.querySelector<HTMLInputElement>('#nide8-id')!.value.trim();
    const username = container.querySelector<HTMLInputElement>('#nide8-user')!.value.trim();
    const password = container.querySelector<HTMLInputElement>('#nide8-pass')!.value;
    if (!server_id || !username || !password) return toast('请填写统一通行证信息', 'warning');
    try {
      const taskId = await bridge.call<string>('start_nide8_login', { server_id, username, password });
      loginState = { taskId, kind: 'authlib', status: '正在验证统一通行证…' };
      render(container, reload);
    } catch (error) {
      toast(errorMessage(error, '无法开始通行证登录'), 'error');
    }
  });
