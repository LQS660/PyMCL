...
class VersionSetupDialog(MessageBoxBase):
    def __init__(self, backend, instance: str, version: str, parent=None):
        ...
        self.win_mode.setCurrentText(
            "全屏" if data.get("window_mode") in FULLSCREEN_MODES else "窗口")
        self.win_w = LineEdit()
        self.win_w.setPlaceholderText("留空则用设置页的全局分辨率")
        self.win_w.setText(str(data.get("window_width") or ""))
        self.win_h = LineEdit()
        self.win_h.setPlaceholderText("留空则用设置页的全局分辨率")
        ...
        form.addRow("窗口宽度", self.win_w)
        form.addRow("窗口高度", self.win_h)
