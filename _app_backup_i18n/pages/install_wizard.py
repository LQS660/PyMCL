...
class InstallWizardDialog(MessageBoxBase):
    def __init__(self, backend, mc_version: str, instance: str, parent=None):
        ...
        hint = BodyLabel("主加载器只能选一个。Forge 可同时勾选 OptiFine（放入 mods）。", self)
        ...
        self.primary.addItems(["无（原版）", "Fabric", "Forge", "Quilt", "NeoForge"])
        self.loader_ver = ComboBox()
        self.loader_ver.setMinimumWidth(280)
        self.loader_ver.addItem("最新")
        row = QHBoxLayout()
        row.addWidget(BodyLabel("主加载器"))
        row.addWidget(self.primary, 1)
        lay.addLayout(row)
        row2 = QHBoxLayout()
        row2.addWidget(BodyLabel("加载器版本"))
        row2.addWidget(self.loader_ver, 1)
        lay.addLayout(row2)

        self.optifine = CheckBox("同时安装 OptiFine（Forge / 原版）")
        self.liteloader = CheckBox("同时安装 LiteLoader（1.7–1.12）")
        self.skip_assets = CheckBox("跳过资源文件校验（加快重装）")
        self.skip_assets.setChecked(bool(CONFIG.get("skip_assets")))
        self.of_ver = ComboBox()
        self.of_ver.addItem("最新")
    ...
    def _sync(self):
        ...
        of_ok = primary.startswith("无") or primary == "Forge"
    ...
    def payload(self) -> dict:
        primary = self.primary.currentText()
        loader = "无" if primary.startswith("无") else primary
