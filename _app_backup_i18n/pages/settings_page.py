...
class SettingsPage(QWidget):
    def __init__(self, backend, parent=None):
        ...
        self.lang_card, self.lang_box = _combo_card(
            ...
            self.backend.available_languages().get(self.backend.get_language(), "简体中文"))
        ui_group.addSettingCard(self.lang_card)
        # 主题包
        theme_card = SettingCard(
            FIF.PALETTE if hasattr(FIF, "PALETTE") else FIF.EDIT,
            "主题包", "保存/加载当前主题配色")
        self.save_theme_btn = PushButton("保存当前主题")
        self.load_theme_btn = PushButton("加载主题")
        self.del_theme_btn = PushButton("删除主题")
        ...
        mig_card = SettingCard(
            ...
            "官方启动器迁移", "从官方 .minecraft 导入版本和账号")
        self.mig_btn = PushButton("检测并迁移")
        ...
        rec_card = SettingCard(
            FIF.DEVELOPER_TOOLS, "智能推荐", "根据硬件自动推荐内存和 Java 设置")
        self.rec_btn = PushButton("查看推荐")
    ...
    def _save_theme(self):
        ...
        dlg = InputDialog("保存主题包", "主题包名称", text="我的主题", parent=self)
        if not dlg.exec():
            return
        name = dlg.value()
        try:
            self.backend.save_theme(name)
            InfoBar.success("已保存", f"主题包「{name}」已保存", parent=self,
                            position=InfoBarPosition.TOP, duration=2500)
        except Exception as e:
    ...
    def _load_theme(self):
        ...
        if not themes:
            InfoBar.info("提示", "没有已保存的主题包", parent=self,
                         position=InfoBarPosition.TOP, duration=2500)
            return
        from ..widgets import ComboDialog
        items = [t["name"] for t in themes]
        dlg = ComboDialog("加载主题", "选择要加载的主题包", items, parent=self)
        if not dlg.exec():
            return
        name = dlg.value()
        try:
            ...
            InfoBar.success("已加载", f"主题包「{name}」已应用", parent=self,
                            position=InfoBarPosition.TOP, duration=2500)
            win = self.window()
            if hasattr(win, "apply_theme"):
                win.apply_theme()
    ...
    def _del_theme(self):
        ...
        from ..widgets import ComboDialog
        items = [t["name"] for t in themes]
        dlg = ComboDialog("删除主题", "选择要删除的主题包", items, parent=self)
        if not dlg.exec():
            return
        name = dlg.value()
        try:
            self.backend.delete_theme(name)
            InfoBar.success("已删除", f"主题包「{name}」已删除", parent=self,
                            position=InfoBarPosition.TOP, duration=2500)
        except Exception as e:
    ...
    def _migrate_official(self):
        detected = self.backend.detect_official_launcher()
        if not detected:
            InfoBar.info("提示", "未检测到官方启动器数据目录", parent=self,
                         position=InfoBarPosition.TOP, duration=3000)
            return
        dir_path = self.backend.official_launcher_dir()
        versions = self.backend.scan_official_versions()
        from qfluentwidgets import MessageBox
        msg = f"发现官方启动器目录: {dir_path}\n\n发现 {len(versions)} 个版本\n\n要导入吗？"
        box = MessageBox("官方启动器迁移", msg, self)
        if not box.exec():
            return
        try:
            task_id = self.backend.migrate_official_launcher()
            InfoBar.success("迁移中", f"导入任务已启动: {task_id}", parent=self,
                            position=InfoBarPosition.TOP, duration=4000)
        except Exception as e:
            InfoBar.error("迁移失败", str(e), parent=self,
                          position=InfoBarPosition.TOP, duration=4000)

    def _show_recommendation(self):
        try:
            rec = self.backend.get_smart_recommendation()
        except Exception:
            rec = {}
        mem = rec.get("memory_mb", 4096)
        ram = rec.get("total_ram_gb", "?")
        cpu = rec.get("cpu_count", "?")
        from qfluentwidgets import MessageBox
        msg = (f"你的系统: {ram} GB 内存 / {cpu} 核 CPU\n\n"
               f"推荐内存: {mem} MB\n\n"
               "可以到「性能」设置区调整。")
        box = MessageBox("智能推荐", msg, self)
        box.yesButton.setText("应用推荐")
        box.cancelButton.setText("关闭")
        if box.exec():
            self.memory_spin.setValue(mem)
            InfoBar.success("已应用", f"内存已设为 {mem} MB，保存设置后生效", parent=self,
                            position=InfoBarPosition.TOP, duration=3000)
